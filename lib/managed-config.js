...
// The machine label is the one piece of an agent row this plugin cannot
// publish for a machine that runs no plugin: herdr's builtin reads herdr's own
// data, which is the whole row a remote entry with no `$`-token renders
// through. But herdr draws that cell only while the client holds more than one
// machine — with one it omits the label, with two it labels EVERY row, and
// the local ones all read `Local`. A column of `· Local` says nothing a pane's
// presence there did not already say, and it eats the width a split row's
// title needs, so the label hides when it says exactly that. `equals`, not
// `contains`: the comparison is on the label, not the connection, and a
// remote machine genuinely named "Local" — which does not exist — would
// deserve its label more than the local rows deserve their noise.
function machineCell(fg, bold) {
  return `{ token = "machine", fg = "${fg}", bold = ${bold}, dim = false, rules = [{ equals = "Local", hide = true }] }`;
}
  ...
  const agentRow = (vendor) =>
    [
      ...
      `[${[
        ...
        // and without a builtin its row rendered empty. It wears the group
        // header's ink so the label reads as structure, not stray text, and
        // hides the `Local` value (machineCell) — a multi-machine client
        // labels every local row with it.
